...
镜像使用 `python:3.12-slim`，默认进程是 UID 10001。`workspace_agent.container_entrypoint` 只负责容器启动：它验证 `PORT` 是 1 到 65535 的十进制端口，只初始化固定的 `/data/workspace` 和 `/data/traces`，必要时从 root 立即降权为 UID 10001 后再执行 Uvicorn。它不是 Agent CLI。
...
`railway.toml` 通过 Dockerfile 部署并探测 `/health`。创建一个持久卷并挂载为 `/data`；在 Railway Variables 设置公开 HTTPS 域名对应的 `ALLOWED_ORIGIN`，并把模型 API key 作为 Secret 设置。Railway 的卷会以 root 挂载，因此还必须设置 `RAILWAY_RUN_UID=0`。入口只会为 `/data/workspace` 和 `/data/traces` 建目录、验证它们不是 link/reparse/非目录、`chown` 为 UID 10001，然后立即降到 UID 10001 执行应用。发布前备份 `/data`，尤其是 workspace、traces 和 reset journal 相关文件。
...
本 README 的 YAML front matter 已声明 `sdk: docker` 和 `app_port: 8000`。Docker Space 对外固定公开 8000，本镜像不假设 Hugging Face 注入 `PORT`。HF 运行容器时使用 UID 1000；基础镜像中的 `/data` 具有可创建子目录的权限，入口在非 root 时不会尝试 `chown`。在 Space Secrets 设置模型 API key，在 Variables 设置 Space 公开 HTTPS 地址对应的 `ALLOWED_ORIGIN`。没有挂载持久化存储时，workspace 与 trace 会在重启后丢失。
